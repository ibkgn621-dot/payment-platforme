...
async def start_consumer(process_message_func):
    ...
    try:
        ...
        queue = await channel.declare_queue("fraud_payment_events", durable=True)
            ...
            async for message in queue_iter:
                async with message.process():
                    try:
                        ...
                        logger.error(f"Erreur traitement message fraude: {e}")
    except Exception as e:
        logger.error(f"Erreur RabbitMQ fraud consumer: {e}")
